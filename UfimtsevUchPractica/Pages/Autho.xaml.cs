...
namespace UfimtsevUchPractica.Pages
{
    ...
    public partial class Autho : Page
    {
        ...
        private void btnEnter_Click(object sender, RoutedEventArgs e)
        {
            ...
            User user = new User();
            user = UchPracticaEntities.GetContext().User.Where(p => p.UserLogin == login && p.UserPassword == password).FirstOrDefault();
            int usercount = UchPracticaEntities.GetContext().User.Where(p => p.UserLogin == login && p.UserPassword == password).Count();
            ...
            else if (countUnsuccessful == 1)
            {
                ...
                {
                    MessageBox.Show("Вы вошли как: " + user.Role.RoleName.ToString());
                    LoadForm(user.Role.RoleName.ToString(), user);
                }
